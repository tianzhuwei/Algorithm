...
void Graph::Johnson() {
	...
	BellmanFord(GraphHead.size() - 1);

	EdgeNode tempp;
	for (int i = 0; i < GraphHead.size(); i++)
	{
		for (int j = 0; j < (GraphHead.at(i))->size(); j++)
		{
			tempp = GraphHead.at(i)->at(j);
			tempp.weight = tempp.weight + dis[i] - dis[tempp.end];
		}
	}//for
